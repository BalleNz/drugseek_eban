class Prompts:
    ...
    ACTIONS:
    "drug_menu" = если пользователь ищет конкретный пункт препарата
    "drug_search" = строго если запрос содержит только 1-2 слова - название препарата  
    "question" = если пользователь задал вопрос или делится симптомами или хочет стать кем-то: "хочу стать ..."
    "other" = остальное
    "spam" = не связано с медициной
    
    примеры для drug_search:
    asdfgh: пользователь ищет aspirin
    мсм: пользователь ищет methylsulfonylmethane
    ...
    ...
    Формат ответа строго JSON:
    {
        ...
        "drug_name": "<строго название ДВ>",
        "drug_name_ru": "<название препарата на русском >",
        "drug_menu": "<пункт из MENU_ITEMS>"
    }
    
    Инструкция:
    1. Если action не в [drug_search, drug_menu] пропускай все поля кроме action
    """
    ...
    4. Напиши 5 препаратов самые эффективные
