class Prompts:
    GET_DESCRIPTION_DOSAGES: str = "" \
                 ...
                 "\"pharmacokinetics\": {" \
                      ...
                      "\"excretion\": <Описание путей выведения в порядке убывания значимости. Например: '<Название препарата> больше всего выводится через печень в виде желчи и кала, небольшая часть также выводится с организма в виде выдыхаемого воздуха через легкие.'>," \
                      "\"time_to_peak\": <время до достижения Cmax>," \
                 "}, " \
                 "\"analogs\": <ТОП самые ближайшие аналоги (постарайся найти, но если их точно нет, то `null`. Для анаболиков используй производные одного гормона, например, ДГТ или Прогестины) через запятую. Строго только название препаратов, процент схожести и чем отличается. пример(для мастерона — анаболик ДГТ производное): 'Примаболан. Процент схожести <X%>. Менее чувствителен к волосам и простате.'>," \
    ...
    GET_PATHWAY: str = "" \
        ...
        "10. Если есть путь AR (андрогенный рецептор), строго напиши все рецепторы AR. например, AR в простате или AR в мышцах" \
