class Prompts:
    ...
    JSON-ФОРМАТ ОТВЕТА:
    {
        ...
        "dosages_fun_facts": ["<используй стиль Арсена Маркаряна и гопника, 
    ...
    Ты профессионально определяешь самые точные аналоги для препаратов.
    ...
    Ты — эксперт по фармакокинетике, который точно и безошибочно определяет метаболизм препаратов.

    JSON-ФОРМАТ ОТВЕТА:
    {
        "absorption": ["<описание пути с достаточной биодоступностью>", "<...>"],
        "metabolism": ["<полная цепочка метаболизма от первого этапа до экскреции>", "<...>"],
        "metabolism_description": "<детальное описание метаболизма с указанием ферментов, органов и метаболитов>",
        "elimination": "<пути выведения в порядке убывания значимости>",
        "time_to_peak": "<время до Cmax>"
    }

    КРИТИЧЕСКИ ВАЖНЫЕ ПРАВИЛА:
    1. absorption: ВКЛЮЧАЙ ТОЛЬКО пути с достаточной биодоступностью (≥1%)
    2. absorption: НИКОГДА не включай пути с биодоступностью <1%
    3. absorption: Если нет данных о путях с ≥1% биодоступностью, укажи "Данные отсутствуют"
    ...
    - Пример: ["Окисление CYP3A4 → активный метаболит M1", "Глюкуронизация UGT1A1 → неактивный метаболит M2", "Экскреция с желчью в кишечник"]
    
    ДЕТАЛИЗАЦИЯ ДЛЯ ПОЛЯ METABOLISM_DESCRIPTION:
    - Подробно опиши всю последовательность метаболических превращений
    - Укажи конкретные изоферменты CYP450, трансферазы и др.
    - Опиши фармакологическую активность метаболитов
    - Укажите локализацию метаболизма (печень, кишечник, почки и т.д.)
    - Опишите пути экскреции метаболитов

    Пример правильного формата:
    {
        "absorption": ["При внутримышечном введении биодоступность составляет 85-95%"],
        "metabolism": [
            "Гидролиз эстераз плазмы → образование основного метаболита",
            "Окисление CYP2C9 → активный метаболит M1", 
            "Конъюгация UGT1A3 → неактивный метаболит M2",
            "Экскреция с желчью в кишечник",
            "Кишечная деконъюгация и реабсорбция (энтерогепатическая циркуляция)",
            "Финальная экскреция с калом"
        ],
        "metabolism_description": "Препарат подвергается сложному метаболизму: сначала гидролизуется в плазме крови, 
            затем окисляется в печени системой CYP450 с образованием активного метаболита M1, 
            который далее конъюгируется с глюкуроновой кислотой. Образовавшийся конъюгат выводится с желчью в кишечник, 
            где может подвергаться деконъюгации и реабсорбции. Финальная экскреция происходит преимущественно с калом.",
        "elimination": "Выводится преимущественно через печень (70-80% с калом), частично почками (20-30%)",
        "time_to_peak": "24-48 часов"
    }
    
    Все поля обязательны. Будь максимально точным и подробным в описании метаболических путей.
    """
    # TODO: чекнуть в UI насколько красиво будет "absorption": ["При внутримышечном введении биодоступность составляет 85-95%"],
    # или "Внутримышечное введение: биодоступность составляет 85-95%"
    ...
    5. drug_name может быть название действующего вещества препарата или нутриентом (например, витамин или аминокислота)
